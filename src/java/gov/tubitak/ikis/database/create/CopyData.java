        ...
        for (int i = 1; i < 6; i++) {
            ...
            try {
                ...
                for (int j = 1; j < sheet.getRows(); j++) {
                    ...
                    VERI_GIR_DUZ1 = convert(sheet.getCell(10, j).getContents());//10
                    ...
                    String KAYNAKID = sheet.getCell(71, j).getContents();//70 
                    String KAYNAK = sheet.getCell(72, j).getContents();//70
                    ENUM_VAS_TIPI = sheet.getCell(74, j).getContents();//74
                    String BASLIKENG = sheet.getCell(75, j).getContents();//74
                    String PROPERTY = sheet.getCell(76, j).getContents();//74
                    String SECTORENG = sheet.getCell(77, j).getContents();//74
                    ResultSet search = Sparql.search("select ?id where{?res rdf:type :Resource. ?res :name \""+KAYNAK+"\". ?res :id ?id.}");
                    List<QuerySolution> toList = ResultSetFormatter.toList(search);
                    if(toList.isEmpty()){
                        try {
                            Sparql.insertProperty(TUIK+"r"+KAYNAKID, RDF+"type", TUIK+"Resource");
                            Sparql.insertLiteral(TUIK+"r"+KAYNAKID, TUIK+"name", KAYNAK);
                            Sparql.insertLiteral(TUIK+"r"+KAYNAKID, TUIK+"id", KAYNAKID);
                        } catch (DatabaseConnectException ex) {
                            Logger.getLogger(CopyData.class.getName()).log(Level.SEVERE, null, ex);
                        } catch (DatabaseWriteOrDeleteException ex) {
                            Logger.getLogger(CopyData.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    }
                    search = Sparql.search("select ?name where{?res rdf:type :Meeting. ?res :id \""+IL_KOOR_TOP_ID+"\". ?res :name ?name.}");
                    toList = ResultSetFormatter.toList(search);
                    if(toList.isEmpty()){
                        try {
                            Sparql.insertProperty(TUIK+"m"+IL_KOOR_TOP_ID, RDF+"type", TUIK+"Meeting");
                            Sparql.insertLiteral(TUIK+"m"+IL_KOOR_TOP_ID, TUIK+"name", IL_KOOR_TOP);
                            Sparql.insertLiteral(TUIK+"m"+IL_KOOR_TOP_ID, TUIK+"id", KAYNAKID);
                        } catch (DatabaseConnectException ex) {
                            Logger.getLogger(CopyData.class.getName()).log(Level.SEVERE, null, ex);
                        } catch (DatabaseWriteOrDeleteException ex) {
                            Logger.getLogger(CopyData.class.getName()).log(Level.SEVERE, null, ex);
                        ...
                        String id=random.nextInt(100000000)+"";
                        try {
                            Sparql.insertProperty(TUIK+"t"+id, RDF+"type", TUIK+"TurkeyValue");
                            Sparql.insertProperty(TUIK+"t"+id, TUIK+"hasSector", TUIK+SECTORENG);
                            Sparql.insertProperty(TUIK+"t"+id, TUIK+"hasResource", TUIK+"r"+KAYNAKID);
                            Sparql.insertProperty(TUIK+"t"+id, TUIK+"hasMeeting", TUIK+"m"+IL_KOOR_TOP_ID);
                            Sparql.insertLiteral(TUIK+"t"+id, TUIK+"id", "t"+id);
                            Sparql.insertLiteral(TUIK+"t"+id, TUIK+"maxValue", ALABILECEGI_EN_FAZLA_DEGER);
                            Sparql.insertLiteral(TUIK+"t"+id, TUIK+"minValue", ALABILECEGI_EN_AZ_DEGER);
                            ...
                            Sparql.insertProperty(TUIK+"s1"+id, TUIK+"hasMeeting", TUIK+"m"+IL_KOOR_TOP_ID);
                            Sparql.insertLiteral(TUIK+"s1"+id, TUIK+"id", "t"+id);
                            Sparql.insertLiteral(TUIK+"s1"+id, TUIK+"maxValue", ALABILECEGI_EN_FAZLA_DEGER);
                            Sparql.insertLiteral(TUIK+"s1"+id, TUIK+"minValue", ALABILECEGI_EN_AZ_DEGER);
                            Sparql.insertLiteral(TUIK+"s1"+id, TUIK+"year", YIL);
                            Sparql.insertLiteral(TUIK+"s1"+id, TUIK+"value", DUZEY1_VERISI);
                            Sparql.insertLiteral(TUIK+"s1"+id, TUIK+"proName", TUIK+PROPERTY);
                            Sparql.insertProperty(TUIK+"s1"+id, TUIK+"hasStage", TUIK+VERI_GIR_DUZ1.toLowerCase().replaceAll("-", ""));
                            ...
                            Sparql.insertProperty(TUIK+"s2"+id, TUIK+"hasMeeting", TUIK+"m"+IL_KOOR_TOP_ID);
                            Sparql.insertLiteral(TUIK+"s2"+id, TUIK+"id", "t"+id);
                            Sparql.insertLiteral(TUIK+"s2"+id, TUIK+"maxValue", ALABILECEGI_EN_FAZLA_DEGER);
                            Sparql.insertLiteral(TUIK+"s2"+id, TUIK+"minValue", ALABILECEGI_EN_AZ_DEGER);
                            Sparql.insertLiteral(TUIK+"s2"+id, TUIK+"year", YIL);
                            Sparql.insertLiteral(TUIK+"s2"+id, TUIK+"value", DUZEY2_VERISI);
                            Sparql.insertLiteral(TUIK+"s2"+id, TUIK+"proName", TUIK+PROPERTY);
                            Sparql.insertProperty(TUIK+"s2"+id, TUIK+"hasStage", TUIK+VERI_GIR_DUZ2.toLowerCase().replaceAll("-", ""));
                            ...
                            Sparql.insertProperty(TUIK+"c"+id, TUIK+"hasMeeting", TUIK+"m"+IL_KOOR_TOP_ID);
                            Sparql.insertLiteral(TUIK+"c"+id, TUIK+"maxValue", ALABILECEGI_EN_FAZLA_DEGER);
                            Sparql.insertLiteral(TUIK+"c"+id, TUIK+"minValue", ALABILECEGI_EN_AZ_DEGER);
                            Sparql.insertLiteral(TUIK+"c"+id, TUIK+"id", "t"+id);
                            Sparql.insertLiteral(TUIK+"c"+id, TUIK+"year", YIL);
                            Sparql.insertLiteral(TUIK+"c"+id, TUIK+"value", IL_VERISI);
                            Sparql.insertLiteral(TUIK+"c"+id, TUIK+"proName", TUIK+PROPERTY);
                            Sparql.insertProperty(TUIK+"c"+id, TUIK+"hasStage", TUIK+VERI_GIR_IL.toLowerCase().replaceAll("-", ""));
...
//                                Sparql.insertLiteral(TUIK+"e"+id, TUIK+"year", YIL);
//                            }
                        } catch (DatabaseConnectException ex) {
                            Logger.getLogger(CopyData.class.getName()).log(Level.SEVERE, null, ex);
                        } catch (DatabaseWriteOrDeleteException ex) {
