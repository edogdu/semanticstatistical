...
public class CopyProvince {
    ...
    public static void read() throws IOException {
        ...
        File file = new File("C:\\Users\\cem.ozkan\\Documents\\ikis - Sektor Verileri ve Aciklamalari\\iller.xls");
        ...
        try {
            ...
            for (int j = 1; j < sheet.getRows(); j++) {
                ...
                VERI_GIR_IL_ID = convert(sheet.getCell(0, j).getContents());//0
                VERI_GIR_IL = convert(sheet.getCell(1, j).getContents());//1
                VERI_GIR_IL_IBBS = convert(sheet.getCell(2, j).getContents());//2
                try {
                    Sparql.insertProperty(TUIK + VERI_GIR_IL.toLowerCase(), RDF + "type", TUIK + "City");
                    Sparql.insertLiteral(TUIK + VERI_GIR_IL.toLowerCase(), TUIK + "id", VERI_GIR_IL_IBBS);
                    Sparql.insertLiteral(TUIK + VERI_GIR_IL.toLowerCase(), RDFS + "label", VERI_GIR_IL);
                ...
                System.out.println(VERI_GIR_IL_ID + " " + VERI_GIR_IL + " " + VERI_GIR_IL_IBBS);
            }
//                for (int j = 1; j < sheet.getRows(); j++) {
//                    
